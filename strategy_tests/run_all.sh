...
find cert_* -type f -name \*.cert | xargs -n 128 awk '/SIGINT/{R="SIGINT"} /s UNSAT/{R="UNSAT"} /s SAT/{R="SAT"} /process-time/{print FILENAME,R,$(NF-1)}' | sort -nk3 > sorted_runtimes.txt
